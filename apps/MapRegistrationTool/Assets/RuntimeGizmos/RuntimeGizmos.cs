...
namespace Placeframe.MapRegistrationTool
{
    public static class RuntimeGizmos
    {
        ...
        public static void DrawCircle(Vector3 position, Vector3 normal, float radius, Color color, bool zTest = true)
                => DrawGizmo(Prefabs.TorusMesh, position, Quaternion.LookRotation(normal), Vector3.one * radius, color, zTest);
        ...
        public static void DrawPlane(Vector3 position, Vector3 xDirection, Vector3 yDirection, Vector2 size, Color color, bool zTest = true)
        {
            ...
            DrawGizmo(Prefabs.QuadMesh, position, Quaternion.LookRotation(zDirection, yDirection), new Vector3(size.x, size.y, 1), color, zTest);
            DrawGizmo(Prefabs.QuadMesh, position, Quaternion.LookRotation(-zDirection, yDirection), new Vector3(size.x, size.y, 1), color, zTest);
        }
        ...
        public static void DrawArrow(Vector3 origin, Vector3 direction, Color color, float thickness, bool zTest = true)
        {
            ...
            DrawGizmo(Prefabs.CylinderMesh, origin, rotation, new Vector3(thickness, thickness, stemLength), color, zTest);
            DrawGizmo(Prefabs.ConeMesh, origin + (direction.normalized * stemLength), rotation, new Vector3(coneLength * 0.25f, coneLength * 0.25f, coneLength), color, zTest);
        }
        ...
        public static void DrawHalfTorus(Vector3 position, Vector3 lookDirection, Vector3 up, float radius, Color color, bool zTest = true)
            => DrawGizmo(Prefabs.HalfTorusMesh, position, Quaternion.LookRotation(lookDirection, up), Vector3.one * radius, color, zTest);
    }
}