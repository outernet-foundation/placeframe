...
the wire-types `MapLocalization` and `CameraFrame`). It touches no
Unity-engine types (no `MonoBehaviour`, no `UnityEngine.Time`, no
`GameObject`). The asmdef `Placeframe.Core` at
...
`packages/csharp/` exists but holds only abandoned stub directories
(`packages/csharp/Placeframe/` and `packages/csharp/Placeframe.Tests/`,
each containing only empty `bin/` and `obj/` subdirectories — no source,
no .csproj). The repo has no live non-Unity .NET projects today.
...
1. **By Unity**, as a precompiled DLL referenced from
   `Plerion.VPS.asmdef` (same pattern Unity already uses for
   `MathNet.Numerics.dll`). The `VisualPositioningSystem.cs` Unity-engine
   glue continues to own subscription wiring, the slew tick subscription,
   the HTTP `Localize()` call, and the platform-specific `ICameraProvider`
   plumbing; it just calls into the portable library for the actual math.
...
- `packages/unity/Placeframe/Assets/Package/MagicLeap/Runtime/MagicLeapCameraProvider.cs`
  ...
  heuristic backstop.
- `packages/unity/Placeframe/Assets/Package/Core/Runtime/Plerion.VPS.asmdef`
  — adds a precompiled reference line for `Placeframe.Filter.dll`.
