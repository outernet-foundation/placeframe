...
### Layout
    ...
    (external) outernet-foundation/release-devkit             -- publish-packages / create-release / ensure-release-pr / fetch-ci-artifacts / mirror-images
...
- [`release-devkit`](https://github.com/outernet-foundation/release-devkit) — the publication machinery behind `publish-packages` / `create-release` / `ensure-release-pr` / `fetch-ci-artifacts` / `mirror-images` (tag ledger, path-diff detection, ephemeral version patching, per-registry feed adapters); `build/publish-config.json` is its declarative input.
