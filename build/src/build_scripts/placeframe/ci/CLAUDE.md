...
- **Built Docker images**: Tagged with the git commit SHA (`service:${GIT_SHA}`), referenced in compose files via `${GIT_SHA:?err}`. `.env.lock` contains only base/third-party digests, never built-image digests.
