...
> Design for transient-content masking — the OneFormer variant, the COCO transient-class list, and the keypoint fallback. When masking ships, the hardened parts fold into `packages/python/neural-networks/AGENTS.md`.
...
- `packages/python/neural-networks/AGENTS.md` — where the masking design
  folds once it lands; existing model-loader patterns set the shape for
