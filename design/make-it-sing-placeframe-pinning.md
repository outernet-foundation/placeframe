...
> Design doc for the make-it-sing↔placeframe version-pinning integration (the `bump-placeframe` command). It concerns the sibling `make-it-sing` repo but is deliberately homed in placeframe's org-level `design/` tier, which welcomes cross-repo design.
