...
The server stack (API, localizer, reconstructor, Keycloak, SeaweedFS, Postgres, Loki/Alloy/Grafana) is a set of Docker microservices under `workloads/`. The reconstructor pulls work via API lease endpoints — there is no separate orchestrator service. See `workloads/AGENTS.md` for service inventory, data flow, and authentication model. For debugging — querying Loki, inspecting S3 buckets, hitting Postgres directly — see the Debugging section in `workloads/AGENTS.md`.
...
10. **Don't `uv sync` inside a service directory**: Running `uv sync` in e.g. `workloads/api/` clobbers the workspace venv. Always sync from the repo root with `uv sync --all-packages`, then re-run `uv run lock-python`.
11. **Syncing PyTorch into the workspace venv**: The `neural-networks` package declares `torch`/`torchvision` behind conflicting `cpu`/`cuda`/`rocm` extras (one per accelerator), so a plain `uv sync --all-packages` does not install them. To get PyTorch (and pycolmap, which neural-networks pulls transitively) into the workspace venv — needed for `workloads/localizer/tests/`, `dirtorch/test_dir.py`, and `dump_openapi` for the localizer's OpenAPI spec — pass the matching extra: `uv sync --all-packages --extra cuda` (or `cpu` / `rocm`). This Claude Code environment has GPU passthrough, so use `--extra cuda`.
