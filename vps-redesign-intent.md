...
**1. Confidence values (`tight`, `loose`) are constants** — same for great and garbage poses. A confidence-based gate at the localizer or filter would treat all results identically. *Band-aid*: a hard floor on raw metrics (`MIN_NUM_INLIERS = 50`, `MIN_INLIER_COVERAGE = 0.15` in `docker/localizer/src/localize.py`) rejects garbage results before they propagate. Replaced by `if metrics.confidence.loose < calibration.loose_min: raise LocalizationError(...)` once a real calibration is fit. Thresholds (`loose_min`, `tight_min`) live on the `CalibrationArtifact`, not as localizer module constants — corpus calibration replaces them via `global.json` only, with no localizer code edit.
