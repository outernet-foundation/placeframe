...
**Observability**: `loki/` (log storage, monolithic mode), `alloy/` (OTLP collector), `grafana/` (query UI). Services emit OpenTelemetry logs over OTLP/gRPC to Alloy (`alloy:4317`), which forwards them to Loki's native OTLP ingestion endpoint (`/otlp/v1/logs`). The Loki log line is the OTel body (the human message); the indexed-resource subset (`service.name`, `deployment.environment.name`) becomes Loki labels and everything else (severity, scope, log attributes, `service.instance.id`) becomes queryable structured metadata, per `docker/loki/config.yaml`'s `otlp_config`. Loki writes to a `loki` bucket in the shared MinIO.
...
**Identity in disabled mode.** When `AUTH_MODE=disabled`, the API reads an optional `X-Anonymous-Identity` request header and synthesizes `AuthenticationResult(user=header_value, auth={"sub": header_value})`, defaulting the identity to `"anonymous"` when the header is absent. The header is never *required* — disabled mode demands nothing of the client. Unity clients still send `SystemInfo.deviceUniqueIdentifier` so per-device attribution survives even without auth. There is no fake JWT signing; the header is the transport. `AuthMiddleware` runs in both modes (it is installed unconditionally), so `connection.user` is always populated — the JWT `sub` under keycloak, the device id or `"anonymous"` under disabled — and no route handler carries a null-identity case.
...
The fourth combination, `http://…` + `keycloak`, is rejected at compose startup: sending OAuth credentials in cleartext is a footgun, and the air-gap deployment shape exists specifically to remove the cert-distribution and OAuth ceremony. The guard lives in `build/src/build_scripts/placeframe/modes.py` and keys off the parsed `PUBLIC_URL` scheme — token-on-the-wire is the actual hazard, not LAN-vs-tunnel.
