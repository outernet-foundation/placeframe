...
- **Reconstruction export is a self-contained bundle, not a row-only tar.** Moving a map between backends (`GET /reconstructions/{id}/tar`, `POST /reconstructions/tar`, driven by `uv run reconstruction`) carries the whole `capture_session → reconstruction → localization_map` graph in one v2 tar: the reconstruction artifacts, the source capture's stored tar nested verbatim as a `capture.tar` member, and all three DB rows in `metadata.json`. Import recreates the graph with the original ids and foreign keys, so an imported map keeps its real source images and real georegistration — required for cross-backend work like localizing one scan's frames against another scan's map. A v1 (reconstruction-only) tar — produced by `--no-capture` or by older exports — still imports via a fallback that synthesizes a stand-in capture session and an identity-pose map. `localization_map_camera_positions` are re-derived from the map on import; `localization_evaluations` are genuinely per-backend (a calibration corpus keyed on `pipeline_version` etc.) and are never transported. Standalone capture transport (`uv run capture`) uses the same `capture.tar` + row-metadata format for moving a capture without its reconstruction.
