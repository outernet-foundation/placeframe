...
**Identity in disabled mode.** When `AUTH_MODE=disabled`, the API reads an `X-Anonymous-Identity` request header and synthesizes `AuthenticationResult(user=header_value, auth={"sub": header_value})`. Unity clients send `SystemInfo.deviceUniqueIdentifier`. There is no fake JWT signing; the header is the transport.

**Client-side auth mode.** There is no `/config` discovery endpoint. Unity clients set `useKeycloak` locally (a boolean Unity-side setting alongside `apiUrl` / `username` / `password`; toggled in the login UI, which hides the username/password fields when off) and branch their API-client wrapper at login: Bearer attachment when on, `X-Anonymous-Identity` attachment when off. A client/server mismatch surfaces as a 401/403 from the first `/api/*` call rather than at login time — an acceptable failure mode given the tradeoff of removing a probe round-trip and a server endpoint.
