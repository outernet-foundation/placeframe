...
def test_committed_persists_across_reload(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    importlib.reload(src.state)
    src.state.cursor_store.set_pending("inode:200")
    src.state.cursor_store.ack_and_get_committed("inode:200")

    importlib.reload(src.state)
    assert src.state.cursor_store.committed == "inode:200"
...
def test_pending_resets_to_committed_on_reload(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    importlib.reload(src.state)
    src.state.cursor_store.set_pending("inode:100")
    src.state.cursor_store.ack_and_get_committed("inode:100")
    src.state.cursor_store.set_pending("inode:200")  # handed out but not acked
    ...
    committed = src.state.cursor_store.ack_and_get_committed("inode:200")
    assert committed == "inode:100"
