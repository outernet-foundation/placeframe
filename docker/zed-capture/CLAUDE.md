...
Box containers log via OTLP (gRPC) to `OTEL_EXPORTER_OTLP_ENDPOINT` (`http://127.0.0.1:4317`, the box-side Alloy `aoa-alloy`), which forwards to the box-side Loki `aoa-loki` (`127.0.0.1:3100`) via Loki's native OTLP ingestion. The log line is the OTel body; `service.name` becomes the `service_name` Loki label, so box logs land under `service_name="zed-capture"`; the box's hardware serial rides as structured metadata `service_instance_id` (from `ZED_BOX_ID`, set by `install-zed`).

The phone forwards box logs to the backend Loki via `LogDrainController` (`apps/CaptureTool/Assets/Scripts/Capture/LogDrainController.cs`). While logged in, it queries `aoa-loki` over the AOA pipe (`GET /loki/api/v1/query_range?query={service_name=~".+"}`, cursor-based on nanosecond timestamps, 1h max lookback) and pushes the returned streams to `{apiUrl}/loki/api/v1/push` (through host-Caddy, authed by the shared backend handler). Timestamps are restamped to phone wall-time at drain — the box boots at 1970 with no RTC, so the offset is `phone_now` minus the box clock read from the query response `Date` header. The query response merges labels and structured metadata into one dict (native OTLP ingestion), so the drain splits it back into canonical labels (`service_name`, `deployment_environment_name`) plus a structured-metadata 3rd value element before pushing, keeping the backend label set low-cardinality. `service_name="zed-capture"` survives the hop and is read with the standard tool:
