...
|   |-- keyframes.py       Lucas-Kanade parallax-based keyframe pre-pass per rig
|   |-- rig.py             Parses frames.csv (gravity-only or position+gravity), applies axis
|   |                      conventions, builds ColmapRigConfig
|   |-- pairs.py           Sequential + retrieval pair generation, plus intra-frame stereo pairs
...
**Priors-on vs priors-off is a structural consequence of the rig.** Multi-camera captures (any rig with more than one camera) run priors-off: stereo baseline anchors metric scale, the BA `PosePrior` loss is disabled, and the final standalone BA pass keeps `sensor_from_rig` pinned so the 7-DOF gauge can't dissolve baseline-supplied scale. Monocular captures run priors-on: position priors are the only metric-scale and absolute-pose source. The decision is computed by the reconstructor from `Rig.is_multi_camera` (`docker/reconstructor/src/reconstructor/run_reconstruction.py`) — there is no API-side default and no `ReconstructionOptions` toggle. Multi-camera rigs additionally drop any `translation` the parser produced from `frames.csv`, so legacy 7-column ZED captures predating the gravity-only schema can't leak drifted VIO positions into pair generation's retrieval-distance gate. Monocular captures conversely reject 3-column gravity-only `frames.csv` at rig construction.

**`frames.csv` schema is column-count-dispatched, then rig-structure-filtered.** Three layouts share one parser (`rig._parse_frame_pose`): 3 columns `timestamp_ms,gx,gy,gz` (gravity only), 6 columns `timestamp_ms,tx,ty,tz,gx,gy,gz` (position + gravity), and 7 columns `timestamp,tx,ty,tz,qx,qy,qz,qw` (legacy position + quaternion; rotation read and discarded). The `(gx, gy, gz)` triple is the unit down vector expressed in that frame's local rig coordinates, derived capture-side from the SDK's `world_from_rig` rotation and the SDK world's down axis. The capture upload validator (`docker/api/src/routers/capture_sessions.py:_validate_monocular_rigs_have_position_priors`) rejects 3-column files for monocular rigs at upload time; multi-camera rigs accept any of the three column counts and the reconstructor nullifies any translation that came in.

**Keyframe selection by visual parallax.** Each rig runs an offline Lucas-Kanade pre-pass over its ref-camera images (downsampled to 320×240 grayscale). A frame becomes a keyframe when accumulated median pixel displacement since the previous keyframe exceeds `keyframe_parallax_threshold_px` (default 50), or when LK tracking collapses (<10 valid points), or unconditionally for the first frame. Hardware-agnostic and VIO-independent — works the same on captures whose `frames.csv` carries no position columns. The pre-pass runs once before feature extraction; per-rig output filters `frame_poses` down to the kept set and the rest of the pipeline sees only keyframes. OpenCV (`cv2.goodFeaturesToTrack` + `cv2.calcOpticalFlowPyrLK`) does the substantive work; the wrapper in `keyframes.py` adapts the numpy types.
...
**Pair generation is sequential + retrieval; pair acceptance is not gated by priors.** Two pair sources: sequential (each frame paired with the next `sequential_window` frames in its rig's timestamp order — COLMAP `SequentialMatcher` / hloc `pairs_from_sequential` equivalent) builds the temporal backbone and catches rapid intra-frame rotation and stationary moments with sub-baseline-threshold motion; retrieval (top `retrieval_neighbors` by global-descriptor cosine similarity, max-pooled over per-image tiles, gated by `retrieval_min_score` plus `retrieval_min_distance_m` *when* position priors are present so visually-similar near-neighbors that sequential already covers don't double up) adds loop closures between visually-similar but distant frames. On the multi-camera priors-off path `frames.csv` carries no positions, so `retrieval_min_distance_m` has no signal to gate against and retrieval emits all top-K candidates; downstream sequential dedup handles overlap. Two-view RANSAC is trusted as-is — no prior-rotation gate between `estimate_two_view_geometry` and the database write.
