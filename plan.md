...
| 3 | End-to-end testing and calibration (code + single-capture starter; multi-capture corpus deferred) | E2E+calibration | In progress — execution chunks 1–3 of 7 landed (harness repair, map-quality metrics, Procrustes pose-error labeling) |
...
4. **`fit_calibration.py`** — pool rows; sklearn logistic + isotonic; `scipy.optimize.minimize` for α/β; Brier + calibration_curve report; artifact write; unit tests (Procrustes solve, α/β solve, round-trip). `pytransform3d` likely added here for SE(3) `log` on the 6-D residuals fed to the α/β fit.
   - Open prerequisite: `scripts/src/scripts/test_placeframe_e2e.py` currently builds `(metrics, err_t_m, err_r_deg, …)` rows in memory but doesn't persist them. Chunk 4 needs to either (a) add a row-emission sink to the harness (JSONL on disk or S3) and have `fit_calibration.py` read from it, or (b) couple the fit script to a harness invocation that hands rows over directly. Resolve before writing the fit script — the answer determines `fit_calibration.py`'s input contract.
