...
| 3 | End-to-end testing and calibration (code + single-capture starter; multi-capture corpus deferred) | E2E+calibration | In progress — execution chunks 1–4 of 10 landed (harness repair, map-quality metrics, Procrustes pose-error labeling, `fit_calibration.py`). Chunks 5–7 (architectural refactor: decouple harness from calibration, backend-rooted inputs) and chunks 8–10 (runtime feature plumbing, starter run, band-aid removal) remain. |
...
5. **Held-out frames as a first-class `ReconstructionOptions` field.** *(NEW — architectural refactor)* The harness today forks captures (downloads, modifies in memory to drop frames, re-uploads as a new capture session) because the reconstructor has no way to skip frames at build time. That ownership of input data is the load-bearing reason `run_e2e.py` ended up with `placeframe-test-captures/` as its source of truth and an `e2e-results.json` intermediate file as its output contract. Fix the API gap and the script refactor (chunk 7) becomes mechanical.
   - Add `held_out_frame_timestamps: list[int] | None = None` to `ReconstructionOptions` in `packages/python/core/src/core/reconstruction_options.py`. Type is `list[int]` because `frames.csv` timestamps are Unix milliseconds (`long timestampMilliseconds = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()` in Unity's `CaptureManager.cs`). **No `reconstructions` table column added** — `ReconstructionOptions` already round-trips through MinIO via `ReconstructionManifest.options` (written at create-time by `docker/api/src/routers/reconstructions.py`, read at run-time by `docker/reconstructor/src/reconstructor/run_reconstruction.py`). The manifest is the single source of truth for what was requested.
   - `docker/reconstructor/`: when reading the capture tar from MinIO, filter rows out of `frames.csv` whose first column (parsed as int) is in `manifest.options.held_out_frame_timestamps` and skip the matching `rig0/camera*/{timestamp}.jpg` files. Apply at the parse boundary — `run_reconstruction.py:104-115` constructs `Rig` objects by passing `frames.csv` text into `Rig.__init__`; either pre-filter the text before that call, or pass the held-out set into `Rig.__init__` and filter inside the existing `for frame in frames_csv.splitlines()[1:]` loop in `rig.py:64`. The image-skip step belongs at `run_reconstruction.py:140-144` where `image_list` is materialized from `rig.frame_poses` — naturally drops the held-out images since their poses won't be in `frame_poses` after the filter.
   - Run `uv run generate-datamodels`, then `uv sync --all-packages && uv run lock-python`, then `uv run generate-clients --config openapi-projects.json --project docker/api`. Generated artifacts ride this commit. (No SQL change → `generate-datamodels` likely produces no diff, but run it to be sure.)
   - Tests: a `docker/reconstructor/tests/` unit test that builds a tiny in-memory `frames.csv` string, calls the filter step with a `held_out_frame_timestamps` list, asserts those rows are gone from the resulting `Rig.frame_poses`.
   - *Pause point — after this, the reconstructor can build maps with frames excluded; nothing else changes yet.*
6. **`localization_evaluations` cache table + API endpoints.** *(NEW — architectural refactor)* Localizing a held-out frame against a reconstruction is deterministic given the keyed inputs *plus* `pipeline_version` (the localizer's git SHA, baked into its image) — neural-net forward passes plus pycolmap RANSAC seeded explicitly in chunk 7 (see "Determinism" note there). Persisting evaluation outcomes lets `fit_calibration.py` re-fit cheaply without re-running localizations.
   - New table `localization_evaluations` keyed by `(reconstruction_id, frame_timestamp bigint, retrieval_top_k, ransac_threshold, pipeline_version text)`. `pipeline_version` is part of the key so that re-running after a localizer code change accumulates rows alongside historical data rather than overwriting it; `fit_calibration.py` filters to one `pipeline_version` at fit time. Columns: the localizer outputs the fit consumes (`inlier_ratio`, `reproj_error_median`, `num_inliers`, `num_correspondences`, `num_matches`, `inlier_coverage`, `pnp_covariance jsonb`, `query_image_diagonal_px`), the truth-error labels (`err_t_m`, `err_r_deg`, `se3_residual jsonb`), and `succeeded boolean`. Schema lives in a new `database/24_localization_evaluations.sql`.
   - API endpoints in `docker/api/src/routers/`: `POST /reconstructions/{id}/localization-evaluations` to upsert one evaluation; `GET /reconstructions/{id}/localization-evaluations` to list all evaluations for a reconstruction (with optional `pipeline_version` query param to filter at the API layer). Tenant-scoped via the existing RLS pattern. The harness writes; the fit script reads.
   - Run `uv run generate-datamodels`, `uv sync --all-packages && uv run lock-python`, `uv run generate-clients --config openapi-projects.json --project docker/api`.
   - Tests: a `docker/api/tests/` integration test for the upsert/list round-trip including the unique-key constraint and `pipeline_version`-filtered list semantics.
   - *Pause point — after this, the backend has the cache schema and endpoints; scripts are unchanged.*
7. **Script refactor: decouple harness from calibration.** *(NEW — architectural refactor)* With chunks 5 and 6 in place, both scripts collapse to thin orchestrators over backend ids.
   - Rename `scripts/src/scripts/run_e2e.py` → `scripts/src/scripts/tune_reconstruction.py`. Strip its purpose to *only* the Plackett-Burman recon-options sweep. Input becomes `--captures <id> [<id>...]` (backend `capture_session_id`s); output becomes a tuning report comparing PB cells. Delete all calibration-corpus emission code from this script. Update `pyproject.toml` script entry: `test-placeframe-e2e` → `tune-reconstruction`.
   - Rewrite `scripts/src/scripts/fit_calibration.py` so a single invocation does the whole thing: `uv run fit-calibration --captures <id> [<id>...] --pipeline-version <sha>` selects held-out frames per capture, discovers/creates reconstructions with `held_out_frame_timestamps` set, localizes the held-out frames, writes results to `localization_evaluations`, reads them back as the corpus, fits, writes `global.json`. Add `--reconstructions <id> [<id>...]` as an alternative input mode that fits against pre-built reconstructions. Add `--no-fit` to populate the cache without fitting.
   - **Held-out-frame selection — pluggable selector with a stride starter.** Define a `HeldOutFrameSelector` protocol/callable: `(frames_csv: str, options: HeldOutSelectionOptions) -> list[int]` returning Unix-ms timestamps. Ship one implementation, `StrideHeldOutSelector`, that pulls `target_count = 100` (overridable via `--held-out-count`) frames at an even temporal stride: `stride = max(1, len(timestamps) // target_count); selected = timestamps[stride // 2 :: stride]`. The selector is invoked by name (default `stride`) so a future `--held-out-selector spatial-bin` lands without touching anything outside the selector module. **Required big comment block** in this module explaining: (a) the simplest temporal-stride approach is the chosen starter — deterministic, scales to capture length, gives roughly even spatial coverage on smooth capture paths; (b) known limitations — does not protect against connectivity loss (held-out frames may be ones the SfM needed for tracks), does not actively distribute spatially in non-smooth captures, fixed count rather than fraction-of-redundant-frames; (c) ideas for later — post-build filter (drop held-outs the SfM unregistered, augment from registered pool), spatial-bin (voxelize positions and pick one per bin), hybrid stride+voxel, count-as-fraction-of-registered. The architectural shape now (selector behind a name) is the load-bearing piece — adding a new strategy must not require surgery to `fit_calibration.py`'s orchestration loop or the localization-evaluations contract.
   - **Reconstruction reuse — match on full `ReconstructionOptions` blob.** For each capture, list reconstructions via `GET /capture-sessions/{id}/reconstructions`, filter to `orchestration_status = 'succeeded'`, fetch each candidate's `manifest.json` from MinIO, and reuse iff `manifest.options == requested_options` (Pydantic equality on the full blob, including `held_out_frame_timestamps`). The "requested options" are constructed by the script from `ReconstructionOptions(held_out_frame_timestamps=selected_timestamps)` — i.e. server defaults for every other knob. If no match: create a new reconstruction with those exact options and wait for it to succeed. **Required big comment block** in the reuse function explaining: (a) full-blob match is the chosen invariant — calibrations must be fit against one pipeline configuration, mixing recons built with different options into one corpus contaminates the fit; (b) ideas for later — options-hash column on `reconstructions` to skip the manifest fetch, opt-in `--match-options-on=held_out_only` flag for development workflows where the operator knows other options are immaterial.
   - **Determinism in the localizer (cache-key contract).** Add a one-time seed at the top of `localize_image_against_reconstruction` in `docker/localizer/src/localize.py`: `pycolmap._core.set_random_seed(LOCALIZER_RANDOM_SEED); torch.manual_seed(LOCALIZER_RANDOM_SEED)` with `LOCALIZER_RANDOM_SEED = 0` as a module constant. Do **not** enable `torch.backends.cudnn.deterministic` or `CUBLAS_WORKSPACE_CONFIG` — those carry a 10–30% latency cost and the residual non-determinism (last-digit drift in conv outputs) is below the discrete inlier-set threshold the fit cares about. The seed change is a localizer code change, so it bumps `pipeline_version` automatically; the cache key includes `pipeline_version`, so old non-deterministic rows from before this commit don't get treated as if they followed the new contract.
   ...
   - Delete the harness-side tar manipulation (`_prepare_capture`, `WithheldFrame`, modify-and-reupload). The harness no longer touches tars.
   - Delete the `placeframe-test-captures/` sibling-directory dependency (it lives outside the repo; just removing the code that reads from it is sufficient — there is nothing to delete in-tree).
   - Update `scripts/tests/test_fit_calibration.py` to mock the API client / localization-evaluations responses instead of building `E2EResults` instances. Five-test coverage stays the same. Add a sixth test for `StrideHeldOutSelector` (deterministic output for a known timestamp list).
   - *Pause point — after this, code is fully decoupled. fit_calibration is a one-shot command. The intent doc's "corpus-gathering spec" rewrites in this commit too (no more sibling directory; captures live in the backend via the normal upload path).*
8. **Runtime feature plumbing + Σ_meas decoupling + per-map loader** — build a real `Features` instance from transformed metrics + map quality features and pass it to `apply_global_calibration`; replace `PnP_cov / tight²` with `α · PnP_cov + β · I`; lazy MinIO per-map loader with global-only fallback. *Pause point — after this, code is logically complete but not yet servable; next chunk produces the calibration that makes it live.*
