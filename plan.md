...
9. **Run starter** — bring up Docker stack; the existing capture session in the backend is reused, but the existing reconstruction for it (and any `LocalizationMap` deployed against it) is **discarded first** so chunks 5–8 actually run (the existing reconstruction predates `held_out_frame_timestamps` and the chunk-2 map-quality metrics, so it would either fail `load_map`'s strict manifest check or skip the held-out-frames path entirely). Discard is **API-only**: `DELETE /localization_maps/{lm_id}` first (its 409-on-LM-exists guard on the reconstruction delete is intentional safety, not a bug to bypass), then `DELETE /reconstructions/{id}`. The reconstruction-delete endpoint is extended this chunk to **cascade S3 cleanup** of the `{id}/` prefix in `placeframe-reconstructions` — that's a real bug (orphaned bytes after row deletion is a leak that exists today; chunk 9 is just where it surfaces). Land the cascade as its own commit, before the discard, with `Squash candidate: belongs in <sha-of-original-delete-endpoint-commit> "<subject>"` so the squash pass folds it into the original delete-endpoint commit. The LM delete endpoint stays DB-only — LMs don't own an S3 prefix; their data lives in DB rows (`LocalizationMapCameraPosition`) and the alignment transform on the row itself. Then `uv run fit-calibration --captures <id> --held-out-count 30` (auto-detects `pipeline_version` from the localizer's `/version` endpoint). 30 held-out frames instead of the default 100: the starter artifact is known-bad and gets replaced by the multi-capture corpus run (deferred follow-up); this run only needs enough samples for chunk 10 to read `TIGHT_MIN` from the success-cluster distribution. Inspect fit report; commit `config/calibration/global.json` with a known-bad-starter `_note` field embedded inside the JSON itself (top-level key, ignored by the loader since `CalibrationArtifact` doesn't validate unknown keys, travels with the artifact across `git mv`).
   - **Fix-attribution workflow.** Chunk 9 is the first end-to-end run of the new architecture against real data; expect surprises. As issues surface and get fixed, each fix lands as its **own commit** with a descriptive subject (not a `fixup!` placeholder). At commit time, scan `git log` over the chunk-5–8 range and decide whether the fix logically belongs in a prior commit — if it does, append a `Squash candidate: belongs in <sha> "<subject>"` line to the message body so a later autosquash pass can fold it in. Do not run the squash during chunk 9 — keep the fix history readable while running, and let a single later pass rewrite to a clean topology. Squashing uses `git commit --fixup=<sha>` + `GIT_SEQUENCE_EDITOR=true git rebase -i --autosquash <sha>~1` per the shared commit-style guide; the in-message `Squash candidate:` annotations are the trail the squash pass follows.
