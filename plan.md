...
| 3 | End-to-end testing and calibration (code + single-capture starter; multi-capture corpus deferred) | E2E+calibration | In progress — execution chunks 1–4 of 7 landed (harness repair, map-quality metrics, Procrustes pose-error labeling, `fit_calibration.py`) |
...
- `docker/localizer/src/build_metrics.py` — `apply_global_calibration(calibration, features=Features.zeros())` zero-valued `Features` placeholder. Replaced with a real `Features` instance built from transformed metrics + map quality features in Phase 3 chunk 5. Same file: `Σ_meas = PnP_cov / tight²` band-aid replaced by `Σ_meas = α · PnP_cov + β · I` reading α/β from the calibration artifact (Phase 3 chunk 5); `CONFIDENCE_TIGHT_FLOOR` removed.
...
- `packages/python/core/src/core/calibration.py` — `IDENTITY_BOOTSTRAP_SENTINEL` and the equality-check skip in `load_global_calibration`. Both removed in Phase 3 once the starter calibration ships.
