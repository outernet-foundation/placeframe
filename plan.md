...
9. **Run starter** — bring up Docker stack; the existing capture session in the backend is reused, but the existing reconstruction for it is **discarded first** so chunks 5–8 actually run (the existing reconstruction predates `held_out_frame_timestamps` and the chunk-2 map-quality metrics, so it would either fail `load_map`'s strict manifest check or skip the held-out-frames path entirely). Discard means deleting both the `reconstructions` row (`DELETE /reconstructions/{id}`) and the `{id}/` prefix in the MinIO reconstructions bucket — the API's delete endpoint only removes the row, so MinIO cleanup is a separate `mc rm --recursive` or `s3_client.delete_objects` call. Then `uv run fit-calibration --captures <id>` (auto-detects `pipeline_version` from the localizer's `/version` endpoint); inspect fit report; commit `config/calibration/global.json` with a known-bad-starter header.
