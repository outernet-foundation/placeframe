...
| 3 | End-to-end testing and calibration (code + single-capture starter; multi-capture corpus deferred) | E2E+calibration | In progress — execution chunks 1–5 of 10 landed (harness repair, map-quality metrics, Procrustes pose-error labeling, `fit_calibration.py`, held-out-frames as a `ReconstructionOptions` field). Chunks 6–7 (architectural refactor: `localization_evaluations` cache table, harness/fit-script decouple) and chunks 8–10 (runtime feature plumbing, starter run, band-aid removal) remain. |
...
6. **`localization_evaluations` cache table + API endpoints.** *(NEW — architectural refactor)* Localizing a held-out frame against a reconstruction is deterministic given the keyed inputs *plus* `pipeline_version` (the localizer's git SHA, baked into its image) — neural-net forward passes plus pycolmap RANSAC seeded explicitly in chunk 7 (see "Determinism" note there). Persisting evaluation outcomes lets `fit_calibration.py` re-fit cheaply without re-running localizations.
   - New table `localization_evaluations` in `database/24_localization_evaluations.sql`, keyed by `(reconstruction_id, frame_timestamp bigint, retrieval_top_k, ransac_threshold, pipeline_version text)`. `pipeline_version` is part of the key so that re-running after a localizer code change accumulates rows alongside historical data rather than overwriting it; `fit_calibration.py` filters to one `pipeline_version` at fit time. Value columns: localizer-output features the fit consumes (`inlier_ratio`, `reproj_error_median`, `num_inliers`, `num_correspondences`, `num_matches`, `inlier_coverage`, `pnp_covariance double precision[]` — 6×6 row-major flat (36 elements), `query_image_diagonal_px`), truth-error labels (`err_t_m`, `err_r_deg`, `se3_residual double precision[]` — length 6), and `succeeded boolean`. Use typed `double precision[]` for the matrix/vector columns rather than `jsonb`; the existing schema has zero `json`/`jsonb` columns and stays uniformly typed.
   - **POST upsert via `INSERT ... ON CONFLICT DO UPDATE`** on the unique 5-tuple. This is a cache table — second write with the same key is a refresh, not a conflict. The `pipeline_version`-in-key invariant already protects against silent overwrites caused by committed code changes; uncommitted-iteration overwrite is on the operator. Avoids forcing the harness loop to GET-first / handle-409 / DELETE-then-POST around 100s of frames per capture.
   - **RLS: tenant policy only.** No `placeframe_orchestration_user` bypass — only `tune_reconstruction.py` and `fit_calibration.py` write, both as a tenant user via the Keycloak flow. State-sync doesn't touch this table. Add the orchestrator bypass later iff a service ever needs to write evaluations cross-tenant.
   - **No supplementary indexes.** The unique constraint's index serves `WHERE reconstruction_id = ?` as a prefix scan; the `WHERE reconstruction_id = ? AND pipeline_version = ?` query filters in-memory after that scan, trivial at expected row counts (~100 frames × ~20 reconstructions = a couple thousand rows per filtered scan). Add an explicit `(reconstruction_id, pipeline_version)` index iff a future query plan turns sour.
   - API endpoints in `docker/api/src/routers/localization_evaluations.py`: `POST /reconstructions/{id}/localization-evaluations` (upsert one evaluation), `GET /reconstructions/{id}/localization-evaluations?pipeline_version=...` (list, with optional `pipeline_version` query param to filter at the API layer). Mounted under `/reconstructions/{id}` to mirror the existing per-reconstruction sub-resource shape (`/manifest`, `/status`, `/points`, `/frame_poses`). The harness writes; the fit script reads.
   - Run `uv run migrate-database`, `uv run generate-datamodels`, `uv sync --all-packages && uv run lock-python`, `uv run generate-clients --config build/openapi-projects.json --project docker/api`.
   - Tests: `docker/api/tests/test_localization_evaluations.py` — HTTP-level integration test against the live api container in this dev env (Keycloak token fetched at fixture setup with dev `user`/`password` creds; same shape as chunk 5's MinIO test). Round-trip: POST a row, GET it back, POST again with the same key (verify upsert overwrites), POST with a different `pipeline_version` (verify both rows coexist), GET filtered by `pipeline_version` (verify one row returned). Skips cleanly if the API isn't reachable.
