...
async def upsert_localization_evaluation(
    ...
    update_columns = {k: v for k, v in values.items() if k != "id"}

    stmt = pg_insert(LocalizationEvaluation).values(**values)
    stmt = stmt.on_conflict_do_update(constraint="localization_evaluations_cache_key", set_=update_columns).returning(
