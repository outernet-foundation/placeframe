...
**This package must never depend on `build-scripts` or `common`.** It is consumed cross-repo via git URL, and uv's `tool.uv.sources` are not transitive: a consumer resolves this package's dependencies by name, from PyPI, unless the consumer declares its own source for them. Every dependency must therefore be PyPI-resolvable — except [`bashrun`](https://github.com/outernet-foundation/bashrun), which consumers declare a git source for alongside this package (the distinctive name keeps PyPI squatting out of the resolution path). `common` is doubly excluded: an unrelated `common` exists on PyPI, and it drags boto3/litestar/opentelemetry that build tooling doesn't need.
