...
**Single shared auth helper.** `api_auth.authenticated_api_client()` is an async context manager that resolves the API host, fetches a Keycloak token when the backend reports `keycloak` mode, and yields a `DefaultApi`. All the transport CLIs (`fit_calibration`, `tune_reconstruction`, `reconstruction`, `capture`) use it. The token is injected as a default header rather than via `Configuration(access_token=...)` because the openapi-generator output emits empty `_auth_settings` on every method — the documented `access_token` kwarg has no effect end-to-end. This is a generator-config gap (every consumer of `placeframe_api_client` has the same blind spot), worked around locally because fixing the generator config is out of scope for this directory.

**Host resolution prefers the local gateway.** `_resolve_host()` probes `http://127.0.0.1:{GATEWAY_PORT}` and uses it when it answers, falling back to `PUBLIC_URL` otherwise. A script run on the backend's own machine then talks to the gateway directly instead of routing large requests (e.g. a 100–300 MB reconstruction-bundle upload) out through the public tunnel relay and back. `PLACEFRAME_API_URL` overrides both for an explicit target (e.g. a container IP). The local probe is plain `http` regardless of `PUBLIC_URL`'s scheme, since the gateway publishes its cleartext h2c listener on `GATEWAY_PORT`.
