...
def sync_schema(project: Path, database: str, log: bool = False):
    print(f"Syncing schema for project: {project}, database: {database}")

    database_path = project / "database"
    models_path = project / "models" / "src" / "models"
    ...
    result = run_command(
        f"{pg_schema_diff_path} apply --from-dsn {migrator_dsn} --to-dir {database_path} --skip-confirm-prompt --allow-hazards {','.join(allow_hazards)}",
        log=log,
    )

    if "Schema matches expected. No plan generated\n" in result:
...
def _generate_models_for_schema(database_schema: str, models_path: Path, service_dsn: str, log: bool) -> None:
    ...
    server_managed_field_names = ("tenant_id", "created_at", "updated_at")

    print(f"Generating table models for schema: {database_schema}")

    run_command(
        f"uv run sqlacodegen --schema {database_schema} --options use_inflect {service_dsn} --outfile {generated_table_models_path}",
        log=log,
    )
    ...
    for table_name, table in tables.items():
        ...
        for column in columns:
            ...
            if column.name in server_managed_field_names:
                continue
...
def _find_pg_schema_diff() -> str:
    # 1) explicit override
    exe = os.environ.get("PG_SCHEMA_DIFF")
    if exe and shutil.which(exe):
        return exe
    # 2) current PATH
    exe = shutil.which("pg-schema-diff")
    if exe:
        return exe
    # 3) common local prefixes
    for p in (
        os.path.expanduser("~/go/bin/pg-schema-diff"),
        os.path.expanduser("~/.local/bin/pg-schema-diff"),
