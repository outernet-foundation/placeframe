...
def _set_host_cable_to(method: str) -> None:
    ...
    # NIC names track PCIe enumeration, so reseating hardware can shift enp6s0 to enp5s0 and
    # leave the saved connection.interface-name pointing at a gone device (nmcli con up then
    # fails opaquely). Rebind to whatever box-facing interface is present now.
...
def _detect_box_interface() -> str:
    # Ethernet device with link carrier whose current NM connection is unbound or an
    # auto-named "Wired connection N" fallback. User-named profiles mark the host's internet
    # path; unmanaged devices (Docker veths, libvirt bridges) are excluded by state.
...
def _evict_competing_connections(device: str) -> None:
    # NM often auto-binds a "Wired connection N" fallback to the box-facing NIC before zedbox
    # is up, racing `nmcli con up zedbox`. Bring competitors down and disable their autoconnect
    # so they don't grab the port again on the next replug.
