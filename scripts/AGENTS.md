...
**`install-zed` disables L4T's USB device-mode service.** NVIDIA's stock `nv-l4t-usb-device-mode.service` pins the USB-C port as a peripheral (CDC ethernet gadget + mass-storage); the AOA host-mode link needs the port available for host duty. `install-zed` `systemctl disable --now`s the unit on every run (idempotent, no-op once disabled). The connectivity model is otherwise pure wired ethernet: SSH targets the box's RJ-45 address at `user@100.64.0.1` with the host's cable-side end at `100.64.0.2`. Every image the box runs — `zed-capture`, `aoa-bridge`, `aoa-gateway`, and the placeframe-owned `loki` / `alloy` wrapper images — is listed in `ZED_SERVICES` and acquired the same way. The default path pulls all five from ghcr.io. The `--build` path replaces ghcr with a transient Docker registry on the host (`docker run -d -p 5000:5000 registry@sha256:...`): a single `docker buildx bake -f compose.zed.bake.yml --push` cross-compiles all five for ARM64 and pushes to it, then the box pulls from `100.64.0.2:5000`. `100.64.0.2:5000` is added to the box-side `/etc/docker/daemon.json` `insecure-registries` so the box's daemon accepts the plain-HTTP pull. Pulls are layer-aware, so iterative dev only ships changed layers across the cable. Building `loki`/`alloy` here rather than pulling them from ghcr is load-bearing: their `tree-<sha>` tag shifts whenever their config or any shared build-context file changes, and a local/unpushed change produces a tag CI never built — pulling it from ghcr fails with `not found`, while building it locally always matches.

**`install-zed` strips the stock JetPack desktop to a headless appliance.** The masked units, the AOA-accessory-dialog respawn bug that motivates the strip, and the choice of strip-the-premise over udev VID filtering are documented on `_strip_to_appliance` in `install_zed/box_install.py` — that block comment is the source of truth, not duplicated here.
...
**`install-zed` uses an SSH control-master multiplexer.** A single TCP connection is established once and reused for every subsequent SSH/scp invocation (`-o ControlMaster=auto -o ControlPath=/tmp/install-zed-ssh-%C -o ControlPersist=120`). Without this, each of the ~20 SSH calls in the deploy would pay the TCP+TLS handshake cost; with it, the deploy completes in roughly one round-trip per command. The control socket is torn down in a `finally` block at the end of the deploy.

**`install-zed` bootstraps first-contact access itself — and the dormancy guard is coupled to `SUDOERS_RULE`.** A virgin Stereolabs box has neither the deploy key nor passwordless sudo, and sudo over ssh has no TTY to prompt on. `_ensure_key_access` (`install_zed/box_install.py`) is the bootstrap: a dormancy guard short-circuits on every bootstrapped box (nothing tried, nothing prompted); otherwise an askpass-driven password session — the factory login tried silently, an operator prompt only as fallback — appends the pubkey to `authorized_keys` and writes `SUDOERS_RULE` to `/etc/sudoers.d/install-zed` via stdin + `sudo install`. The guard probes `sudo -n systemctl --version` **because that command is in `SUDOERS_RULE`'s list** — `sudo -n true` would be denied even on a bootstrapped box (`true` is not in the rule), making the guard never sleep. Corollary: removing a command from `SUDOERS_RULE` (in `install_zed/constants.py`) silently breaks the guard's dormancy — keep the guard's probe inside the rule's list.
...
- [`docker-devkit`](https://github.com/outernet-foundation/docker-devkit) — the standalone package holding the Docker-stack lifecycle (`up`, `down`, `build`) and the SHA/auth helpers, git-referenced from the root `pyproject.toml`. `scripts/` imports from it (`docker_devkit.context_sha.compute_service_shas` in `install_zed/orchestrator.py`, `docker_devkit.modes.parse_env_file` in `api_auth.py`); the inverse does not hold.
